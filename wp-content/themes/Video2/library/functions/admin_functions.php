...
function mytheme_admin_head() { 
  if (isset($_REQUEST['page']) && $_REQUEST['page']=='functions.php') {
	echo '<link rel="stylesheet" type="text/css" href="'.get_template_directory_uri().'/library/functions/admin_style.css" media="screen" />';
	echo '<script type="text/javascript" src="'.get_template_directory_uri().'/library/js/ptthemes.js"></script>';
	echo '<script type="text/javascript" src="'.get_template_directory_uri().'/library/js/jquery-ui.js"></script>';
...
function get_videoes_category_array()
{
	global $wpdb;
	$return_array = array();
	$pn_categories = $wpdb->get_results("SELECT $wpdb->terms.name as name, $wpdb->term_taxonomy.count as count, $wpdb->terms.term_id as cat_ID 
	                            FROM $wpdb->term_taxonomy,  $wpdb->terms
                                WHERE $wpdb->term_taxonomy.term_id =  $wpdb->terms.term_id
                                AND $wpdb->term_taxonomy.taxonomy = 'videoscategory'
								ORDER BY name");	
	foreach($pn_categories as $pn_categories_obj)
	{
		$return_array[] = array("id" => $pn_categories_obj->cat_ID,
